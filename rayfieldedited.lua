...
local function AddDraggingFunctionality(DragPoint, Main)
    pcall(function()
        ...
        -- Use TouchPanGestureRecognizer for mobile compatibility
        local dragRecognizer = DragPoint:WaitForChild("DragRecognizer") or DragPoint:FindFirstChildWhichIsA("TouchPanGestureRecognizer")
        if not dragRecognizer then
            dragRecognizer = Instance.new("TouchPanGestureRecognizer")
            dragRecognizer.Parent = DragPoint
        end

        dragRecognizer.GestureBegan:Connect(function(input)
            Dragging = true
            MousePos = input.Position
            FramePos = Main.Position
        end)

        dragRecognizer.GestureChanged:Connect(function(input)
            local Delta = input.Position - MousePos
            TweenService:Create(Main, TweenInfo.new(0.45, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {Position = UDim2.new(FramePos.X.Scale, FramePos.X.Offset + Delta.X, FramePos.Y.Scale, FramePos.Y.Offset + Delta.Y)}):Play()
        end)

        dragRecognizer.GestureEnded:Connect(function(input)
            Dragging = false
        end)
    end)
end
